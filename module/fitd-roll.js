    ...
    buttons: {
      yes: {
        ...
        callback: (html) =>
        {
          ...
          FitDRoller(dice_amount, position, effect);
        }
      },
      no: {
 ...
 */
export async function FitDRoller(dice_amount, position = "risky", effect = "standard")
{
  ...
  r.roll();
  showChatRollMessage(r, zeromode, position, effect);
}
 ...
 * @param {string} effect effect
 */
async function showChatRollMessage(r, zeromode, position = "", effect = "")
{
  ...
  roll_status = getFitDActionRollStatus(rolls, zeromode);
  ...
  const result = await renderTemplate("modules/foundryvtt-fitdroller/templates/fitd-roll.html", { rolls, roll_status, position, position_localize, effect, effect_localize, zeromode });
 ...
 */
export function getFitDActionRollStatus(rolls, zeromode = false)
{
